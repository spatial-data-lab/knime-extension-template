...
3. Before proceeding in KNIME Analytics Platform we have to create the Python environment with all required packages e.g. `geopandas` to do so execute the following command that uses the {{cookiecutter.extension_name}}.yml file in the local repository clone: 
   ```bash
   #browse to the folder where the {{cookiecutter.extension_name}}.yml file of the local repository clone is located and then execute
   conda env create -f {{cookiecutter.extension_name}}.yml 
   
   
   conda activate {{cookiecutter.extension_name}}
   
   conda info
   # Record the env location path such as D:\ProgramData\Anaconda3\envs\{{cookiecutter.extension_name}} 
   ```
...
6. Go back to the KNIME Analytics Platform and configure the following KNIME Python settings:
   ...
   * KNIME -> File-> Preference> KNIME> Python(Labs) Choose {{cookiecutter.extension_name}} environment

7.  Reopen the KNIME Analytics Platform and you should see the {{cookiecutter.extension_name}} extension in your KNIME node repository.
